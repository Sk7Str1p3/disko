...
std::any parseArgs(int argc, char **argv) {
  ...
  while (true) {
    ...
    case 'h':
      ...
      exit(0);
    case '?':
      std::cerr << BOLDRED "Unknown option >> " RESET << char(optopt)
                << std::endl;
      exit(1);
    }
  }

  // check if flake URL is valid:
  std::regex pattern(R"([^:]+:[^/]+/[^#]+#[^#]+)");
  if (std::regex_match(flake, pattern) == 1) {
    std::cout << "good" << std::endl;
  } else {
    std::cerr << "Flake URL has " RED "wrong " RED "format! " BLUE "Aborting.";
    exit(0);
  }
  ...
  if (!disko_Config.empty() && !flake.empty()) {
...
int main(int argc, char *argv[]) {
  ...
  std::string nix_Args;
  if (!disko_Config.empty()) {
    nix_Args += " --arg"
                " diskoFile ";
    nix_Args += disko_Config;
  }
  if (!flake.empty()) {
